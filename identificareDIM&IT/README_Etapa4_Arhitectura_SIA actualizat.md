...
| Lipsa dataset-urilor publice: Nu există dataset-uri publice cu simboluri tehnice specifice (ISO/STAS) rotite. | Augmentare Geometrică Avansată: Generarea sintetică a datelor prin rotații controlate și păstrarea coordonatelor OBB. | Data Acquisition |
...
|---------------------|-------------------------------------|--------------------------|
| **Etichetare/adnotare manuală** | • Etichetat manual 875 de clase in RoboFLOW | Fișier Excel/JSON cu labels + capturi ecran tool etichetare + log timestamp-uri lucru |

### Contribuția originală la setul de date:

**Total observații finale:** 205 imagini
**Observații originale:** 41 imagini (100%)

**Tipul contribuției:**
[] Date generate prin simulare fizică  
[X] Date achiziționate cu senzori proprii (Scanare/Selecție manuală documentație tehnică)
[X] Etichetare/adnotare manuală (Roboflow) 
[ ] Date sintetice prin metode avansate  

**Descriere detaliată:**
Datele (desenele de execuție) au fost selectate dintr-un suport de curs de "Procese Industriale". Am extras manual 41 de desene reprezentative care conțineau simboluri diverse.

Etichetare: Datele au fost importate în Roboflow și etichetate manual folosind OBB (Oriented Bounding Boxes) pentru a captura corect orientarea cotelor și rugozităților.

Procesare (Modul 1): Am dezvoltat un script Python (src/etl_pipeline.py) care utilizează librăria Albumentations.

Preprocesare: Resize la 640x640 px (Stretch).

Augmentare: Pentru fiecare imagine originală, s-au generat 4 variante noi cu: Rotație (-2° la +2° cu recalcularea coordonatelor poligoanelor), Luminozitate (+/- 15%) și Zgomot Gaussian.

Rezultat: Din 41 imagini originale au rezultat 205 imagini gata de antrenare, împărțite automat în Train (140), Validation (30) și Test (35) pentru a preveni data leakage.


**Locația codului:** `src/data_acquisition/etl_pipeline.py`
**Locația datelor:** `data/generated/` sau `data/raw/original/`

**Dovezi:**
- Log generare date: data_log.csv
---
...
1.[IDLE]: Sistemul așteaptă încărcarea unui fișier (JPG/PNG).

2.[PREPROCESS]: Imaginea este redimensionată la 640x640 px.

3.[AI_INFERENCE]: Modelul YOLOv8 rulează predicția OBB.

4.[USER_GROUPING]: Etapă interactivă unde utilizatorul confirmă asocierile (ex: care rugozitate aparține cărei cote).

5.[GENERATE_PLAN]: Algoritm bazat pe reguli care determină operația tehnologică.

6.[LOG_AND_EXPORT]: Salvarea rezultatelor.

### Tranzițiile critice sunt:

- **[IDLE] → [PREPROCESS]:** Se declanșează automat când utilizatorul încarcă o imagine validă (JPG/PNG).
- **[PREPROCESS] → [AI_INFERENCE]:** Are loc imediat ce redimensionarea la 640x640 px este finalizată cu succes.
- **[AI_INFERENCE] → [USER_GROUPING]:** Tranziție condiționată de detectarea a cel puțin un obiect (num_detections > 0).
- **[AI_INFERENCE] → [IDLE]:** Se activează dacă nu sunt detectate obiecte (eroare/mesaj utilizator).
- **[USER_GROUPING] → [GENERATE_PLAN]:** Declansată manual de utilizator prin butonul de validare a selecției.
- **[GENERATE_PLAN] → [LOG_AND_EXPORT]:** Finalizarea determinării procesului; salvează automat datele pentru re-antrenare și resetează sistemul la starea IDLE.
...
- [X] Cod rulează fără erori: `python src/data_acquisition/etl_pipeline.py` sau echivalent LabVIEW
